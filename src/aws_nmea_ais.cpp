...
c_vdm * c_vdm_dec::decode(const char * str, const long long t)
{
  s_pl * ppl = NULL;
  c_vdm * pnd;
  int i = 0;
...
c_vdm * c_vdm_dec::dec_payload(s_pl * ppl, const long long t)
{
  m_type = (short) ppl->payload[0];
  c_vdm * pnd = NULL;

  for (int i = 0; i < vdm_objs.size(); i++){
    if(vdm_objs[i].match(m_type)){
      pnd = vdm_objs[i].dat;
    }
  }
...
void c_vdm_msg1::dec_payload(s_pl * ppl)
{
  ...
  tmpi = ((dat[10] & 0x1F) << 27) |
    ...
    ((dat[14] & 0x3E) << 3);
  tmpi >>= 4;
  m_lon_min = (tmpi & 0x08000000 ? tmpi | 0xF0000000 : tmpi);
  m_lon = (float) (m_lon_min * (1.0/600000.0));
  ...
  tmpi = ((dat[14] & 0x01) << 31) |
    ...
    ((dat[19] & 0x30) << 1);
  tmpi >>= 5;
  m_lat_min = (tmpi & 0x04000000 ? tmpi | 0xF8000000 : tmpi);
  m_lat = (float) (m_lat_min * (1.0/600000.0));
...
void c_vdm_msg5::dec_payload(s_pl * ppl, const long long t)
{
  ...
  auto payload = 
    CreateStaticAndVoyageRelatedData(builder,
				     ...
				     builder.CreateVector(m_destination, 20),
				     m_to_bow, m_to_stern,
				     m_mmsi,
				     m_imo,
				     m_draught);    
...
void c_vdm_msg6::dec_payload(s_pl * ppl)
{
  ...
  // sequence number (38-39 bit)
  m_seqno = (dat[6] & 0x0C) >> 2;
  ...
  m_mmsi_dst = ((dat[6] & 0x03) << 28) |
    ...
    ((dat[10] & 0x3F) << 4) |
    ((dat[11] & 0x3C) >> 2);
  m_retransmit = (dat[11] & 0x02 != 0);// (70 bit)
  m_dac = ((dat[12] & 0x3F) << 4) |
    ((dat[13] & 0x3C) >> 2);
  m_fid = ((dat[13] & 0x03) << 4) |
    ((dat[14] & 0x3C) >> 2);
